...
export class Modem {
    ...
    constructor(url: string, ca: string | Buffer, cert: string | Buffer, key: string | Buffer, rejectUnauthorized: boolean = true) {
        this.url = url;
        this.ca = ca;
        this.cert = cert;
        this.key = key;
    ...
    async listContianers(all: boolean = true, limit: number = 0) {
        const rurl = join(this.url, '/v1.41/containers/json?' + new URLSearchParams({all: all.toString(), limit: limit.toString()}).toString());
        const resp = await ((await fetch(rurl, { agent: this.agent })).json()) as Array<{Id:string}>;
    ...
    async pullImage(image: string, tag: string) {
        ...
        const rurl = join(this.url, '/v1.41/images/create?' + new URLSearchParams({ fromImage: image, tag: tag }).toString());
        return (await fetch(rurl, { method: 'post', agent: this.agent })).text();
    }

    async createContainer(image: string, workingDir: string, cmd: Array<string>, user: string, networkMode: string) {
        ...
        const response = await fetch(createUrl, { method: 'post', body: body, headers: { "content-type": "application/json" }, agent: this.agent });
    ...
    async removeContainer(id: string, force: boolean = true) {
        const deleteUrl = join(this.url, '/v1.41/containers/' + id + '?' + new URLSearchParams({ force: force.toString() }).toString());
        const response = await fetch(deleteUrl, { method: 'delete', agent: this.agent });
    ...
    async startContainer(id: string) {
        ...
        const response = await fetch(startUrl, { method: 'post', agent: this.agent });
        if(response.status !== 204) {
            throw new Error(await response.text());
        }
    }
    ...
    async stopContainer(id: string) {
        ...
        const response = await fetch(stopUrl, { method: 'post', agent: this.agent });
        if(response.status !== 204) {
            throw new Error(await response.text());
        }
    }
    ...
    async killContainer(id: string) {
        ...
        const response = await fetch(killUrl, { method: 'post', agent: this.agent });
        if(response.status !== 204) {
            throw new Error(await response.text());
        }
    }
    ...
    async waitContainer(id: string) {
        ...
        const response = await fetch(waitUrl, { method: 'post', agent: this.agent });
    ...
    async logsContainer(id: string, stdout: boolean = true, stderr: boolean = true) {
        const logsUrl = join(this.url, '/v1.41/containers/' + id + '/logs?' + new URLSearchParams({ stdout: stdout.toString(), stderr: stderr.toString() }).toString());
    ...
    async getFs(id: string, path: string){
        const fsUrl = join(this.url, '/v1.41/containers/' + id + '/archive?' + new URLSearchParams({ path: path }).toString());
    ...
    async putFs(id: string, path: string, body: string | NodeJS.ReadableStream | ReadStream) {
        const fsUrl = join(this.url, '/v1.41/containers/' + id + '/archive?' + new URLSearchParams({ path: path }).toString());
        const response = await fetch(fsUrl, { method: 'put', body: body, agent: this.agent });
        if(response.status !== 200) {
            throw new Error(await response.text());
        }
    }
    ...
    async remove(force: boolean = true, later: boolean = true){
        if(later === true) {
            return new Promise((resolve, reject) => {
                setTimeout(() => {
                    this.modem.removeContainer(this.id, force).then(resolve).catch(reject);
                }, 1000);
            });
        }else{
